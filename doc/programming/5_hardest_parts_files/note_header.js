...
var 
  load = self.load || [],
  entryList = [
 ["How to do Software Engineering Contracts", "No really, something that works.", "/software-contracting-howto.html"],
 ["Sidestep Wireless Logins", "Route all traffic through measly little ping packets", "/ICMPTunnelGuide.html"],
 ["The 5 Hardest Parts of Programming", "Learning through pain, practice, and patience", "/The5HardestThings.html"],
 ["Linux and Programming Cheat Sheets", "Various mappings and reminders to get you through the day", "/LinuxKeyboardShortcuts.html"],
 ["The Emperor's New Clothes", "Imagine a world where Bash supports JSON.", "/TheEmperorsNewClothes.html" ],
 ["Piggy Pack", "Hiding things out in the open", "/PiggyPack.html"],
 ["Don't Pay For Content", "They Don't Deserve Your Money", "http://kristopolous.blogspot.com/2012/01/why-pirate.html"],
 ["What a Nice Logo you have", "Where Did You Find it?", "http://kristopolous.blogspot.com/2011/11/what-nice-logo.html"],
 ["The Acid3 of JS has a few surprises.", "IE10 comes out on top.", "http://kristopolous.blogspot.com/2011/11/acid3-of-js-has-few-surprises.html"],
 ["Abandon your cake", "Stop the DIY", "http://kristopolous.blogspot.com/2011/05/replacing-in-house-solutions-with-third.html"],
 ["Finding out when a file is accessed", "A basic inotify utility", "/inotify.html"],
// This one is too controversial
// ["Thinking different.", "A look into the mental illness of a hacker.", "/LoseThos.html"],
 ["Command-fu graphics premier", "Dipping into the various sauciness", "/fade.html"],
 ["Deferring the load", "Script injection and detection", "/javascript_loadlib.html"]
];
function c(type) {
...
load.push(function(){
  var 
    menu = d({id: "menu"}).c("div"),
    title = d({ innerHTML: "&#x25BC; articles", "class": "title", "onclick": "openMenu(this)" }).c("a");

  self.title = title;
  self.list = d({id: "menu-contents"}).c("ul");
  ...
  var 
    ...
    a, 
    entry,
    container = document.getElementById("menu-contents");

  for(var ix = 0; ix < entryList.length; ix++) {
    entry = entryList[ix];
    li = c("li");
    a = d({href: entry[2]}).c("a");
    title = d(entry[0]).c("b");
    a.appendChild(title);
    a.innerHTML += "<br/>" + entry[1];
    li.appendChild(a);
    li.onclick = function(){
      document.location = this.firstChild.href;
    }
    container.appendChild(li);
    if(document.location.toString() == entry[2]) {
...
load.push(function(){
	...
	header.innerHTML = '<a href=writing.html>View All Topics</a>';
