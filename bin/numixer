#!/bin/bash
# numixer - take color from Xresources and used them for the highlights and background   
# !! gtk2 only !!
...
FULLPATH=/usr/local/share/console_colors/$THEME
BACKGROUND=$(grep background $FULLPATH | cut -d":" -f2)
FOREGROUND=$(grep foreground $FULLPATH | cut -d":" -f2)
TEXT=$(grep color7 $FULLPATH | cut -d":" -f2)
TEXT2=$(grep color15 $FULLPATH | cut -d":" -f2)
BASE=$(grep color8 $FULLPATH | cut -d":" -f2)
cat << EOF > gtkrc

# Numix GTK Theme

gtk_color_scheme = "base_color:$BACKGROUND\nbg_color:$BACKGROUND\ntooltip_bg_color:$BASE\nselected_bg_color:$FOREGROUND\ntext_color:$TEXT\nfg_color:$TEXT\ntooltip_fg_color:$TEXT\nselected_fg_color:$TEXT\nmenubar_bg_color:$BASE\nmenubar_fg_color:$TEXT2\ntoolbar_bg_color:$BASE\ntoolbar_fg_color:$TEXT2\nmenu_bg_color:$BASE\nmenu_fg_color:$TEXT2\npanel_bg_color:$BASE\npanel_fg_color:$TEXT2\nlink_color:#000000"
...
style "chrome-gtk-frame" {
	...
	ChromeGtkFrame::scrollbar-slider-prelight-color = shade (0.8, @bg_color)
...
NEWPATH=/usr/share/themes/Numix$THEME/gtk-2.0
sudo mkdir -p $NEWPATH
sudo cp -r /usr/share/themes/Numix/gtk-2.0 $NEWPATH
sudo mv gtkrc $NEWPATH/
