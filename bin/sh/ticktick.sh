...
# this lets you put json in your bash scripts, which is totally a lovely idea
# just throw that inside some backticks
# recommended usage is to put this at the top of your script (right under the shebang):
#
# . ticktick.sh
#
# bash variables (like "$foo") are preserved in ticktick block
# while js can have vars that start with $, obviously ticktick can't
#
# ticktick's strength is in its api; source it at the beginning of other scripts
# and utilize the crap out of it.
#
# tickParse: parses things inline
# tickVars: see current variables
# __tick_var_debug: see interim bash code
