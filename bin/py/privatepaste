...
def privatepaste(content, formatting=None, line_numbers=False, expires='1 month', password=None, subdomain=None):
...
parser.add_option('-s', '--subdomain', dest='subdomain', default=None,
                  help='Specify the subdomain to use')
...
parser.add_option('-D', '--savedefaults', action='store_true', dest='save_defaults', default=False,
                  help='Save the given arguments except --tee/--no-tee as defaults in ~/.privatepaste')
...
config_file = os.path.expanduser('~/.privatepaste')
if options.save_defaults:
    ...
    print 'Options have been saved to ~/.privatepaste'
    sys.exit(0)
...
if os.path.isfile(config_file):
    with open(config_file, 'r') as f:
        for line in f:
            ...
            if key in ['LANGUAGE', 'SUBDOMAIN', 'EXPIRATION', 'PASSWORD', 'LINE_NUMBERS']:
...
link = privatepaste(content.rstrip(), formatting=options.language, line_numbers=options.line_numbers,
                    expires=options.expiration, password=options.password, subdomain=options.subdomain)
