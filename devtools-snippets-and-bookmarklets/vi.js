...
function term_vi_t() {
  ...
  for (i = (cursorx + left) + 1; i < t.length; i++) {
    ...
    if (c == vi_ft_reg) {
      cursorx = (i - left) - 1;
      if (term_vi_flag('d') || term_vi_flag('c') || term_vi_flag('y'))
        cursorx++;
      if (cursorx < 0) {
        left = 0;
        cursorx = i - 1;
...
function term_keyfix(e) {
  ...
  if (e.DOM_VK_UP) {
    if (e.DOM_VK_UP == ch)
      ch = 57373;
    else if (e.DOM_VK_DOWN == ch)
      ch = 57374;
    else if (e.DOM_VK_LEFT == ch)
      ch = 57375;
    else if (e.DOM_VK_RIGHT == ch)
      ch = 57376;
  }
...
function term_keypress_inner(e, synth) {
  ...
  if (e.charCode) {
    ...
    if (e.DOM_VK_UP) {
      if (e.DOM_VK_UP == k)
        k = 57373;
      else if (e.DOM_VK_DOWN == k)
        k = 57374;
      else if (e.DOM_VK_LEFT == k)
        k = 57375;
      else if (e.DOM_VK_RIGHT == k)
        k = 57376;
    } else {
...
function editor_disable(sav) {
  ...
  for (z = document.body.firstChild; z; z = z.nextSibling) {
    if (z.tagName && z._flipe)
      z.style.display = z._orige;
  }
